...
| Playwright a11y / E2E / visual-regression | present | Visual-regression suite (`npm run test:visual`) skipped in this QA window — LKID-81 had landed on main but the suite's macOS-baselined PNGs are known to drift on CI Linux runners (covered by the `update-baselines` workflow_dispatch path). The new design-mode chart elements have testids (`chart-dialysis-markers`, `chart-outcome-gap-wedge`, `chart-starting-egfr`) so future visual-regression baselines will stabilize cleanly. |
| Vitest / Jest | 0 | **N/A** — no JS unit-test runner in `app/package.json`. The `scripts` block has `dev / build / start / lint / test:visual / test:visual:update`, so only the Playwright visual-regression suite is exposed via npm scripts; Playwright a11y/E2E suites also exist (`playwright.a11y.config.ts`, `playwright.e2e.config.ts`, `app/tests/a11y/`, `app/tests/e2e/`), but they are run via `npx playwright test --config=…` with their dedicated configs rather than through npm test scripts. |
