...
| Vitest / Jest | 0 | **N/A** — no JS unit-test runner in `app/package.json`. The `scripts` block has `dev / build / start / lint / test:visual / test:visual:update`; only the Playwright visual-regression suite exists, no unit-test runner. |
