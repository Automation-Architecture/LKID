...
| Vitest / Jest | 0 | **N/A** — no JS unit-test runner in `app/package.json`. The `scripts` block has `dev / build / start / lint / test:visual / test:visual:update`. The repo also has `playwright.a11y.config.ts` and `playwright.e2e.config.ts` alongside the visual-regression config, though only `test:visual` / `test:visual:update` get dedicated npm scripts; the a11y and E2E suites are run via `npx playwright test --config=…` directly. |
