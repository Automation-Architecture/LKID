...
| Scope | Replace the LKID-90 dialysis-crossing marker (6px filled dot in trajectory color + adjacent "Dialysis range — Year N" text) with a 22px (#9F2D2D fill, 2px white stroke) circle holding a centered white Manrope-700 13px "D". Drop year/range label entirely — year info is implicit from x-axis. Same red regardless of which line crossed, so the marker reads as a single concept ("dialysis"). Verdict initially covered a single-file code change (`EgfrChart.tsx` lines 780-856); follow-up commits on the same PR added: this verdict file itself (`agents/yuri/drafts/sprint6-pr82-qa-verdict.md`), 2 regenerated visual-regression baseline PNGs (auto-pushed by the `update-baselines` workflow_dispatch), and a Copilot-driven dead-field removal in `EgfrChart.tsx` (commit `8b16313`). PDF and threshold-line / wedge / starting-eGFR callout / scenario lines untouched throughout. |
...
Implementation matches the approved plan precisely (filled #9F2D2D circle r=11, 2px white stroke, centered white "D" Manrope-700 13px with `dominantBaseline="central"` for vertical centering, year/range label dropped, label-stacking machinery deleted, `chart-dialysis-markers` testid preserved). The plan originally allowed the unused `Crossing.color` and `Crossing.year` fields to remain in the type as cheap retention; on Copilot's first review pass (21:03:55Z) they were flagged as dead data and removed in follow-up commit `8b16313`. Final shipped state: both fields gone from the type and from the push site. All build gates green: `npx tsc --noEmit` clean, `npm run lint` clean (only 1 pre-existing unrelated warning in `SprintTracker.tsx:11` for an unused `STATUS_LABELS` const — present on main, not introduced by this PR), `npm run build` SUCCESS in 4.3s with all 7 routes generated. White-on-#9F2D2D contrast 7.27:1 (AAA, well above WCAG AA 4.5:1). PR-level CI on tip: CodeRabbit SUCCESS, Vercel SUCCESS, visual regression SUCCESS (after baseline regen via `update-baselines` workflow_dispatch — LKID-81 path).
...
| **2** | Contrast — white on #9F2D2D ≥ AA (4.5:1) | PASS | **7.27:1** (AAA). Computed via WCAG 2.1 sRGB formula: `L_white = 1.0`, `L_#9F2D2D ≈ 0.0947`, ratio = `(1.0 + 0.05) / (0.0947 + 0.05) ≈ 7.27`. Same combination already accepted in LKID-90 (PR #71); foreground/background swap doesn't change the contrast ratio (it's symmetric by definition), so the prior "white text on #9F2D2D background" treatment from PR #71 reuses the same 7.27:1 here. |
...
| `Crossing.color` field removed from type | ✓ Removed in follow-up commit `8b16313` per Copilot review | Plan §"Two related items I noticed but did NOT change" #2 originally allowed retention; Copilot review pass at 21:03:55Z flagged the dead data and the fields (`color` + `year`) were dropped from the type and the push site. Final shipped state: neither field is emitted. |
