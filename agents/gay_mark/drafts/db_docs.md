...
### 1a. Table-by-Table Rationale

#### `users`
The patient identity table. Stores demographics needed by the CKD-EPI eGFR formula (sex, date_of_birth) and contact information (email for magic link delivery). **No `password_hash` column** — Decision #1 mandated magic-link-only authentication, eliminating password storage and the associated security surface (bcrypt, password resets, credential stuffing).

`sex` is `NOT NULL` with a CHECK constraint (Decision #3) because the CKD-EPI 2021 formula requires sex as an input. The value `'unknown'` is permitted but triggers a lower confidence tier.

`height_cm` and `weight_kg` are nullable — optional supplementary data that may be used in future phases.

#### `magic_link_tokens`
Implements passwordless authentication. The raw token is emailed to the patient; only the SHA-256 hash is stored. Key properties:
- **Single-use:** `used_at` is set on consumption; application rejects tokens where `used_at IS NOT NULL`.
- **Time-bounded:** `expires_at` enforces a TTL (recommended: 15 minutes).
- **ON DELETE CASCADE:** When a user is deleted, their tokens are automatically removed.

#### `lab_entries`
The core clinical data table. One row per lab visit per user. All CHECK constraints mirror the API validation ranges (Decision #9) as defense-in-depth:

| Field | Range | Required |
|-------|-------|----------|
| bun | 5 - 150 mg/dL | Yes |
| creatinine | 0.3 - 15.0 mg/dL | Yes |
| potassium | 2.0 - 8.0 mEq/L | Yes |
| age_at_visit | 18 - 120 years | Yes |
| hemoglobin | 4.0 - 20.0 g/dL | No (Tier 2) |
| glucose | 40 - 500 mg/dL | No (Tier 2) |
| systolic_bp | 60 - 300 mmHg | No |

Decision #12: Tier 2 confidence requires **both** hemoglobin AND glucose. The schema allows either to be NULL independently — the tier logic lives in the application/prediction engine, not in the DB.

`visit_date` enables multi-visit slope calculation (Tier 3 requires 3+ distinct visit dates). The composite index `(user_id, visit_date)` supports efficient retrieval of a patient's chronological lab history.

Decision #10: No prediction results are stored. The prediction engine reads lab_entries and computes trajectories on demand.

#### `guest_sessions`
Ephemeral storage for unauthenticated users (Decision #4). Lab data is stored as JSONB to avoid coupling guest data shape to the `lab_entries` schema. When a guest creates an account, the application parses the JSONB and inserts proper `lab_entries` rows.

The 24-hour TTL is enforced via `expires_at DEFAULT (NOW() + INTERVAL '24 hours')` and a purge cron job that runs every 15 minutes.

#### `audit_log`
Append-only record of all PHI access and mutations. Required by HIPAA for accountability and breach investigation. Key design choices:

- **BIGSERIAL PK** instead of UUID — optimized for high-volume sequential inserts. Audit logs are write-heavy and rarely queried by primary key.
- **ON DELETE SET NULL** for `user_id` (Decision #14) — audit records must survive user deletion. After a patient exercises their right to delete, we retain the audit trail with a NULL user reference.
- **`session_token`** column supports auditing guest actions before account creation.
- **`details` JSONB** captures context without schema rigidity (changed fields, validation errors, purge counts, etc.).
- **`ip_address` INET** — PostgreSQL's native IP type supports both IPv4 and IPv6.
- **No UPDATE/DELETE grants for `app_service`** — the application can only INSERT into audit_log, never modify or delete records. This preserves audit integrity.

### 1b. Table NOT Included

**`predictions`** — Decision #10 explicitly ruled out prediction result storage for MVP. The prediction engine recomputes trajectories on demand from stored lab_entries. This reduces HIPAA surface area (fewer PHI tables) and avoids cache invalidation complexity. Revisit in Phase 3 if FDA audit requirements demand result provenance.
...
| `idx_audit_log_user_ts` | audit_log | (user_id, created_at) | Audit queries by user + time range |
...
- **Partitioning** on `audit_log` by `created_at` (monthly or quarterly) — if the audit table exceeds 10M rows.
...
### NOT NULL by Default
Every column is `NOT NULL` unless the field is explicitly optional in the product spec. Nullable columns: `name`, `date_of_birth`, `height_cm`, `weight_kg` (user profile optional fields), `hemoglobin`, `glucose`, `egfr_override`, `systolic_bp`, `sglt2_inhibitor`, `upcr`, `upcr_unit`, `ckd_diagnosis` (optional lab fields), `used_at` (magic link lifecycle), and audit_log columns (`user_id`, `session_token`, `resource_id`, `details`, `ip_address`).
...
### 5c. Role-Based Access Control (SPEC-72)
...
### 5d. Audit Logging (SPEC-30, SPEC-76, SPEC-79)
...
### 5e. Data Retention and Deletion (SPEC-73)
...
### 5g. Breach Notification Support
...
## 8. Seed Data Approach (for Yuri's Testing)
...
- HIPAA compliance section (Section 5 above) is ready for co-authoring with Husser during PRD drafting.
