...
- Header + PDF polish (PR #92, open 2026-05-02). `header.tsx` stripped to solid `--kh-navy` bar — no nav links, no text. Copilot flagged: (1) component has no importers (dead code); (2) empty `<header role="banner">` is announced as a landmark by screen readers — needs `aria-hidden="true"` and `role` removed, or routes need to import/wire the component. `PdfReport.tsx` CSS-only scaling: ~188px recovered (header 64→52px, body-pad, h1 28→22px, sec-title margins, chart-card padding, legend margin, sc-card padding/val 28→22px, explain-grid column 170→120px, margin-top). Added `page-break-inside: avoid` to `.sc-cards` and `.explain-grid`.
