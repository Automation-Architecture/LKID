...
**Status:** Sprint 5 COMPLETE + post-sprint hardening in progress. App live at kidneyhood-automation-architecture.vercel.app with no-auth tokenized flow (`/labs` → `/gate/[token]` → `/results/[token]`). G1/G2/G3 guardrails active. CSP enforcing (flipped 2026-04-30). A11y CI suite wired + full-page axe scope restored (LKID-93/94/96, PRs #81/#83/#85). Landing page nav + How It Works section stripped (PR #91, 2026-05-01). PDF single-page fit + header bar polish in review (PR #92).
...
- LKID-83 — PostHog analytics confirmed live (2026-05-01). `NEXT_PUBLIC_POSTHOG_KEY` + `NEXT_PUBLIC_POSTHOG_HOST` set on Vercel production + preview. Four funnel events active: `labs_submitted`, `gate_captured`, `results_viewed`, `pdf_downloaded`.
...
- Landing page nav + How It Works strip (PR #91, merged 2026-05-01). Removed inline `<nav>` with desktop links + hamburger + mobile overlay from `app/page.tsx` (-368 lines). Removed entire "How It Works" 3-step cards section + CSS block. Replaced with a plain `64px --navy` spacer div. Kept `.chip`/`.chip .dot` (used in "future" section).
- Header + PDF polish (PR #92, merged 2026-05-02). `header.tsx` stripped to solid `--kh-navy` decorative `<div aria-hidden="true">` — no nav links, no landmark. Wired into `gate/[token]/layout.tsx`, `results/[token]/layout.tsx`, and new `labs/layout.tsx`; inline `.nav` JSX + CSS removed from all three pages (~71 lines). `PdfReport.tsx` CSS-only scaling: ~188px recovered (header 64→52px, body-pad, h1 28→22px, sec-title margins, chart-card padding, legend margin, sc-card padding/val 28→22px, explain-grid column 170→120px). Added `page-break-inside: avoid` to `.sc-cards` and `.explain-grid`.
- Overlay inset + blank line cleanup (PR #94, merged 2026-05-02). `.kh-gate .overlay` `inset-top` was hardcoded to `64px` (lg header height), causing a gap at mobile (48px) and md (56px). Fixed with responsive overrides: default `48px`, `min-width: 768px → 56px`, `min-width: 1024px → 64px`. Removed cosmetic double blank lines in gate and labs CSS.
